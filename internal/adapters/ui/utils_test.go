...
func TestBuildSSHCommand_PortForwarding(t *testing.T) {
	tests := []struct {
	...
	}{
		{
			...
			expected: []string{"tssh", "-L", "8080:localhost:80", "-L", "3306:db.internal:3306", "user@example.com"},
		},
		{
			name: "remote forward",
			server: domain.Server{
			...
			expected: []string{"tssh", "-R", "8080:localhost:3000", "-R", "*:80:localhost:8080", "user@example.com"},
		},
		{
			...
			expected: []string{"tssh", "-D", "1080", "-D", "localhost:1081", "user@example.com"},
		},
		{
			...
			expected: []string{"tssh", "-L", "8080:localhost:80", "-R", "9090:localhost:9090", "-D", "1080", "user@example.com"},
		},
		{
			...
			expected: []string{"tssh", "-L", "127.0.0.1:8080:localhost:80", "-L", "*:3000:localhost:3000", "user@example.com"},
		},
		{
			...
			expected: []string{"tssh", "-L", "8080:localhost:80", "-o", "ExitOnForwardFailure=yes", "-o", "GatewayPorts=clientspecified", "user@example.com"},
		},
		{
			...
			expected: []string{"tssh", "-L", "8080:localhost:80", "-o", "ClearAllForwardings=yes", "user@example.com"},
		},
	}
	...
	for _, tt := range tests {
		t.Run(tt.name, func(t *testing.T) {
			...
			if !strings.HasPrefix(result, "tssh ") {
				t.Errorf("BuildSSHCommand() should start with 'tssh ', got: %q", result)
			}
		})
	}
}
...
func TestBuildSSHCommand_CompleteCommand(t *testing.T) {
	...
	if !strings.HasPrefix(result, "tssh ") {
		t.Errorf("Command should start with 'tssh ', got: %q", result)
	}
